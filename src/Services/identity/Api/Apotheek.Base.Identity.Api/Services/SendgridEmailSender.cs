...
namespace Apotheek.Base.Identity.Api.Services
{
    public class SendgridEmailSender : IEmailSender
    {
        ...
        public SendgridEmailSender(ILogger logger, ISendGridClient client, SendgridConfiguration configuration)
        {
            _logger = logger;
            _client = client;
        ...
        public async Task SendEmailAsync(string email, string subject, string htmlMessage)
        {
            ...
            } else
            {
                ...
                _logger.Error($"Response with code {response.StatusCode} and body {errorMessage} after sending email: {email}, subject: {subject}");
            }
        }
    }
}
