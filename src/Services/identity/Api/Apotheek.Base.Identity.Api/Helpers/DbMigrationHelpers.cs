...
namespace Apotheek.Base.Identity.Api.Helpers
{
    public static class DbMigrationHelpers
    {
        ...
        /// https://github.com/aApotheek/IdentityServer4.Admin#ef-core--data-access
        /// </summary>
        /// <param name="host"></param>      
        public static async Task EnsureSeedData<TIdentityServerDbContext, TIdentityDbContext, TPersistedGrantDbContext, TAuditLogDbContext, TUser, TRole>(IHost host)
            ...
            where TRole : IdentityRole<long>, new()
        {
            using (var serviceScope = host.Services.CreateScope())
            {
                ...
                await EnsureDatabasesMigrated<TIdentityDbContext, TIdentityServerDbContext, TPersistedGrantDbContext, TAuditLogDbContext>(services);
                await EnsureSeedData<TIdentityServerDbContext, TUser, TRole>(services);
            }
        }

        public static async Task EnsureDatabasesMigrated<TIdentityDbContext, TConfigurationDbContext, TPersistedGrantDbContext, TAuditLogDbContext>(IServiceProvider services)
            where TIdentityDbContext : DbContext
        ...
        where TRole : IdentityRole<long>, new()
        {
            using (var scope = serviceProvider.GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                ...
                await EnsureSeedIdentityServerData(context, rootConfiguration.ServerDataConfiguration);
                await EnsureSeedIdentityData(userManager, roleManager, rootConfiguration.DataConfiguration);
            }
        }
        ...
        private static async Task EnsureSeedIdentityData<TUser, TRole>(UserManager<TUser> userManager,
            RoleManager<TRole> roleManager, DataConfiguration identityDataConfiguration)
            where TUser : IdentityUser<long>, new()
            where TRole : IdentityRole<long>, new()
        {
        ...
        /// </summary>
        private static async Task EnsureSeedIdentityServerData<TIdentityServerDbContext>(TIdentityServerDbContext context, ServerDataConfiguration identityServerDataConfiguration)
            where TIdentityServerDbContext : DbContext, IAdminConfigurationDbContext
        {
            if (!context.IdentityResources.Any())
            {
                foreach (var resource in identityServerDataConfiguration.Resources)
                {
