...
namespace Apotheek.Base.EntityFramework.Identity.Repositories
{
        ...
        where TUserToken : UserIdentityUserToken
    {
        ...
        public virtual async Task<PagedList<TUser>> GetPharmacyUsersAsync(int pharmacyId, string search, int page = 1, int pageSize = 10)
        {
            var pagedList = new PagedList<TUser>();
            Expression<Func<TUser, bool>> searchCondition = x => (x.UserName.Contains(search) || x.Email.Contains(search) && x.PharmacyId==pharmacyId);
        ...
        public virtual async Task<PagedList<TUser>> GetLoginPharmacyUsersAsync(int pharmacyId, string search, int page = 1, int pageSize = 10)
        {
            var pagedList = new PagedList<TUser>();
            var users = DbContext.Set<TUser>()
                .Join(DbContext.Set<TUserToken>(), u => u.Id, ur => ur.UserId, (u, ur) => new { u, ur })
                .Where(t => t.u.PharmacyId.Equals(pharmacyId))
        ...
        public virtual async Task<(IdentityResult identityResult, string roleId)> CreateRoleAsync(TRole role)
        {
            ...
            return (identityResult, Convert.ToString(role.Id));
        }

        public virtual async Task<(IdentityResult identityResult, string roleId)> UpdateRoleAsync(TRole role)
        {
            var thisRole = await RoleManager.FindByIdAsync(Convert.ToString(role.Id));
        ...
        public virtual async Task<(IdentityResult identityResult, string userId)> CreateUserAsync(TUser user)
        {
            try
            {
                ...
                return (identityResult, Convert.ToString(user.Id));
            }
            catch (Exception e)
            {
                Console.WriteLine(e);
        ...
        {
            var userIdentity = await UserManager.FindByIdAsync(Convert.ToString(user.Id));
        ...
        public virtual async Task<PagedList<TUserClaim>> GetUserClaimsAsync(string userId, int page = 1,
            int pageSize = 10)
        {
            var id = ConvertUserKeyFromString(userId);
        ...
        public virtual async Task<PagedList<TRoleClaim>> GetRoleClaimsAsync(string roleId, int page = 1,
            int pageSize = 10)
        {
            var id = ConvertRoleKeyFromString(roleId);
        ...
        {
            var user = await UserManager.FindByIdAsync(Convert.ToString(claims.UserId));
        ...
        {
            var role = await RoleManager.FindByIdAsync(Convert.ToString(claims.RoleId));
