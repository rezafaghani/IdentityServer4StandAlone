...
using Apotheek.Base.BusinessLogic.Identity.Events.Identity;
...
namespace Apotheek.Base.BusinessLogic.Identity.Services
{
    ...
    {
        protected readonly IIdentityRepository<TUserKey, TRoleKey, TUser, TRole, long, TUserClaim, TUserRole, TUserLogin, TRoleClaim, TUserToken> IdentityRepository;
        protected readonly IIdentityServiceResources IdentityServiceResources;
        protected readonly IMapper Mapper;
        protected readonly IAuditEventLogger AuditEventLogger;

        public IdentityService(IIdentityRepository<TUserKey, TRoleKey, TUser, TRole, long, TUserClaim, TUserRole, TUserLogin, TRoleClaim, TUserToken> identityRepository,
        ...
        public virtual async Task<TUsersDto> GetPharmacyUsersAsync(int pharmacyId, string search, int page = 1, int pageSize = 10)
        {
            var pagedList = await IdentityRepository.GetPharmacyUsersAsync(pharmacyId, search, page: page, pageSize: pageSize);
            var usersDto = Mapper.Map<TUsersDto>(pagedList);
        ...
        public virtual async Task<TUsersDto> GetLoginPharmacyUsersAsync(int pharmacyId, string search, int page = 1, int pageSize = 10)
        {
            var pagedList = await IdentityRepository.GetPharmacyUsersAsync(pharmacyId, search, page: page, pageSize: pageSize);
            var usersDto = Mapper.Map<TUsersDto>(pagedList);
        ...
        {
            var identity = await IdentityRepository.GetUserAsync(Convert.ToString(userIdentity.Id));
